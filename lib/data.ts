import type { Resident, DailyData, HealthMetric, Notification, FallData } from "./types"
...
export function generateResidentData(resident: Resident, timeRange: number): DailyData[] {
  ...
  let baseStability = Math.max(0, resident.stabilityScore * 0.7 - 1)
  let basePinkNoiseUsage = resident.pinkNoiseUsage === "High" ? 70 : resident.pinkNoiseUsage === "Medium" ? 40 : 10
  ...
  for (let i = 0; i < timeRange; i++) {
    ...
    // Calculate stability with high variability and a slight upward trend
    const stabilityVariability = (Math.random() - 0.5) * 4 // Increased variability to 4 points
    const stabilityTrend = (i / timeRange) * 1.5 // Slight upward trend
    let stability = Math.max(0, Math.min(10, baseStability + stabilityVariability + stabilityTrend))

    // Calculate pink noise usage with high variability
    const pinkNoiseVariability = (Math.random() - 0.5) * 50 // Increased variability to 50%
    let pinkNoiseUsage = Math.max(0, Math.min(100, basePinkNoiseUsage + pinkNoiseVariability))

    // Introduce random significant changes
    if (Math.random() < 0.1) {
      // 10% chance of significant change
      stability += (Math.random() - 0.5) * 4
      pinkNoiseUsage += (Math.random() - 0.5) * 50
    }

    stability = Math.max(0, Math.min(10, stability))
    pinkNoiseUsage = Math.max(0, Math.min(100, pinkNoiseUsage))

    const fallOccurred = Math.random() < 0.03 // 3% chance of fall
