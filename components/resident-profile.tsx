...
export function ResidentProfile({ initialResident }: { initialResident: Resident }) {
  ...
  const [stabilityGoal, setStabilityGoal] = useState<number | null>(null)
  ...
  const latestData = useMemo(() => getLatestData(selectedResident), [selectedResident])
  ...
  const initialPinkNoiseUsage = useMemo(
    () => generateResidentData(selectedResident, 1)[0].pinkNoiseUsage,
    [selectedResident],
  )
            ...
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <p>
                    <strong>Current Stability Score:</strong> {currentStabilityScore.toFixed(1)}
                    <span className={`ml-2 ${Number(stabilityScoreChange) >= 0 ? "text-green-600" : "text-red-600"}`}>
                      ({stabilityScoreChange}%)
                    </span>
                  </p>
                  <p>
                    <strong>Current Pink Noise Usage:</strong> {currentPinkNoiseUsage.toFixed(1)}%
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <Input
                      type="number"
                      value={stabilityGoal !== null ? stabilityGoal : selectedResident.stabilityGoal}
                      onChange={(e) => {
                        const newValue = Number(e.target.value)
                        setStabilityGoal(newValue)
                      }}
                      step="0.1"
                      min="0"
                      max="10"
